...
public class PointagesVentilationJobTest {
	...
	public void executeInternal_OneVentilTask_success() throws Exception {
		...
		Mockito.verify(ddS, Mockito.times(1)).downloadDocumentAs(String.class, "theventilation/processTask?idVentilTask=" + t1.getIdVentilTask(),
				null);
	}
	...
	public void executeInternal_OneVentilTask_failure() throws Exception {
		...
		IDownloadDocumentService ddS = Mockito.mock(IDownloadDocumentService.class);
		Mockito.doThrow(new Exception("fake exception message")).when(ddS).downloadDocumentAs(String.class, "theventilation/processTask?idVentilTask=12", null);
		...
		Mockito.verify(pDao, Mockito.times(1)).commitTransaction();
		Mockito.verify(pDao, Mockito.times(1)).rollBackTransaction();
		Mockito.verify(ddS, Mockito.times(1)).downloadDocumentAs(String.class, "theventilation/processTask?idVentilTask=12",
				null);
	}
}
