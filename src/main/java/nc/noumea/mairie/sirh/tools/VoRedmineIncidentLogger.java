...
public class VoRedmineIncidentLogger {
	...
	public void addException(Throwable exception, Integer id) {
		...
		for (VoExceptionWithListAgents ex : listException) {
			if (null != exception && ex.getNameException().equals(exception.getClass().getName()) && id != null) {
				ex.addId(id);
				return;
			}
		}

		listException.add(new VoExceptionWithListAgents(exception.getClass().getName(), exception.getMessage(), exception, id));
	}
	...
	public void addException(String nameException, String messageException, Throwable exception, Integer id) {
		...
		// on cherche d abord si l exception existe deja
		for (VoExceptionWithListAgents ex : listException) {
			if (null != nameException && ex.getNameException().equals(nameException)) {
				ex.addId(id);
				return;
			}
		}

		listException.add(new VoExceptionWithListAgents(nameException, messageException, exception, id));
	}
