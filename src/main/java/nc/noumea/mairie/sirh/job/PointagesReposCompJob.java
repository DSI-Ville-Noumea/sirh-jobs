...
public class PointagesReposCompJob extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		do {
			...
			logger.info("Processing ReposCompTask id [{}] for Agent [{}] and idVentilDate [{}], schedulded by [{}]...",
					rcT.getIdRcTask(), rcT.getIdVentilDate(), rcT.getIdAgent(), rcT.getIdAgentCreation());

			try {
				downloadDocumentService.downloadDocumentAs(
						String.class,
						String.format("%s%s%s", SIRH_PTG_WS_Base_URL, SIRH_PTG_WS_ReposCompTaskUrlPart,
								rcT.getIdRcTask()), null);
