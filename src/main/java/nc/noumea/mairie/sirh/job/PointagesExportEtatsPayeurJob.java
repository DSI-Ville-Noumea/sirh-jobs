...
public class PointagesExportEtatsPayeurJob extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		try {
			...
			String url = String.format("%s%s%s", SIRH_PTG_WS_Base_URL, SIRH_PTG_WS_ExportEtatsPayeurTaskUrl, eT.getIdExportEtatsPayeurTask());
			logger.info("Calling url {}...", url);
			downloadDocumentService.downloadDocumentAs(String.class, url, null);

			// Second WS call to mark all pointages as JOURNALISE 
			url = String.format("%s%s%s", SIRH_PTG_WS_Base_URL, SIRH_PTG_WS_ExportEtatsPayeurDoneUrl, eT.getIdExportEtatsPayeurTask());
			logger.info("Calling url {}...", url);
			downloadDocumentService.downloadDocumentAs(String.class, url, null);
		...
		try {
			String url = String.format("%s%s%s", SIRH_PTG_WS_Base_URL, SIRH_PTG_WS_ExportEtatsPayeurDoneStopUrl, eT.getTypeChainePaie());
			logger.info("Calling url {}...", url);
			downloadDocumentService.downloadDocumentAs(String.class, url, null);
		} catch (Exception ex) {
