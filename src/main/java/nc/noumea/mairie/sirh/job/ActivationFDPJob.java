...
public class ActivationFDPJob extends QuartzJobBean {

	private Logger logger = LoggerFactory.getLogger(ActivationFDPJob.class);
	...
	protected void executeInternal(JobExecutionContext jobContext) throws JobExecutionException {
		...
		ActionFDPJob eT = sirhDao.getNextActivationFDPTask();

		if (eT == null) {
			logger.info("Did not find any ActivationFDPTask to process... exiting job.");
			sirhDao.rollBackTransaction();
			return;
		}

		logger.info("Processing ActivationFDPTask id [{}] for idFDP [{} : {}], schedulded by [{}]...",
		...
		try {
			...
			logger.error("An error occured trying to process ActivationFDPTask :", ex);
			eT.setStatut(String.format("Erreur: %s", ex.getMessage()));
			incidentLoggerService.logIncident("ActivationFDPJob", ex.getCause() == null ? ex.getMessage() : ex
		...
		logger.info("Processed ActivationFDPTask id [{}].", eT.getIdActionFdpJob());
	}
}
